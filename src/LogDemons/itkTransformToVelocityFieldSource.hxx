...
::BeforeThreadedGenerateData( void )
{
  ...
  if( this->m_Transform->IsLinear() )
    ...
    itkExceptionMacro(
      <<
      "The transform reports to be linear but is not a subclass of MatrixOffsetTransformBase, TranslationTransform or IdentityTransform. This case is not supported yet.");
    }

  // /\todo should we use TransformToDeformationFieldSource + DisplacementToVelocityFieldLogFilter
  // / automatically in the default case?
  itkExceptionMacro(
    <<
    "Only linear transforms are supported yet. Try and use a TransformToDeformationFieldSource followed by a DisplacementToVelocityFieldLogFilter.");
}
